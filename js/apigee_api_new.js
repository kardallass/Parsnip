...
*/
    this.request = function(verb,request,headers,settings) {
      theApi.makeRequest(theApi.buildRequest(verb,request,headers,settings));
    }

    this.buildRequest = function(verb,request,headers,settings) {
      if (request) {
        ...
        settings.shortType = settings.type.split("?")[0];
        var requestObject = {
          url: settings.endpoint+request+'.'+settings.shortType,
          data: headers,
          type: settings.verb,
          dataType: settings.shortType,
        }
        if (settings.callback) requestObject.callback = settings.callback;
        return requestObject;
      }
    }
    
    this.makeRequest = function(requestObject) {
      var returnObject = {};
      $.extend(requestObject.data, {'X-PINGOTHER':'pingpong','X-Requested-With':'XMLHttpRequest'});
      if (theApi.smartkey) $.extend(requestObject.data, {"smartkey":theApi.smartkey});
      if (requestObject.type != 'get') {
        if (theApi.authorization) $.extend(requestObject.data, {"Authorization":"Basic "+theApi.authorization});
        requestObject.headers = requestObject.data;
        requestObject.data = JSON.stringify(requestObject.data);
      }
      $.extend(requestObject, {
        xhrFields: {
          withCredentials: true
        },
        beforeSend: function(xhr) {
          if (theApi.authorization && requestObject.data.Authorization) xhr.setRequestHeader('Authorization',requestObject.data.Authorization);
        },
        success: function(data,textStatus,jqXHR) {
          returnObject.response_message = textStatus;
        	...
        	if (this.callback && (window[this.callback] && ((typeof window[this.callback]) === 'function'))) window[this.callback](data);
          $.after_request();
        },
        error: function(jqXHR, textStatus, errorThrown) {
          responseMessage = textStatus+" ("+errorThrown+")";
        	...
        	theApi.returnObject = returnObject;
        	try {
        		showResponseMessage('Sorry, that didn’t work. Please check the instructions and try again.');
        	} catch (e) {}
        	$.after_request();
        }
      });
...
*/
    ...
    var uid = (parent.getUID && (typeof parent.getUID == 'function')) ? parent.getUID() : getUrlParam('uid');
